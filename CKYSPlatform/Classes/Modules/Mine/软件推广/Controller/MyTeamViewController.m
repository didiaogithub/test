...
@interface MyTeamViewController ()<UIScrollViewDelegate,UITableViewDelegate,UITableViewDataSource,FinalTeamTableViewCellDelegate,MyTeamTableViewCellDelegate>
{
    UIView *_bankView;
    UIView *_topView;
    UIView *_middleView;
...
-(void)createNavViews {
    ...
    [btn setTitle:@"礼包销售" forState:UIControlStateNormal];
...
-(void)createPreSaleShopButton {
    UIBarButtonItem *right = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"presaleshopicon"] style:UIBarButtonItemStylePlain target:self action:@selector(clickpreSaleButton)];
...
-(void)refreshWithModel:(MyTeamModel *)teamModel{
    ...
    [_customerButton setTitle:[NSString stringWithFormat:@"消费用户%@人",customer] forState:UIControlStateNormal];
...
-(void)getMyTeamListData{
    _customerButton.userInteractionEnabled = NO;
    ...
    [PPNetworkHelper POST:requestUrl parameters:pramaDic responseCache:^(id responseCache) {
        ...
        _customerButton.userInteractionEnabled = YES;
...
-(void)getDataWithDict:(id)json andIsLoad:(BOOL)isLoad{
    ...
    NSArray *listArr = nil;
    if ([_string isEqualToString:@"0"] || [_string isEqualToString:@"1"]) {
...
//    _ckButton = [self createMessageButtonWithNomalImage:[UIImage imageNamed:@"mineteamck"] andTitle:@"0套" andTag:3330 andAction:@selector(clicksButton:)];
...
//    _fxButton = [self createMessageButtonWithNomalImage:[UIImage imageNamed:@"mineteamfx"] andTitle:@"0" andTag:3331 andAction:@selector(clicksButton:)];
    ...
    UILabel *fxline = [[UILabel alloc] init];
    [_middleView addSubview:fxline];
    [fxline setBackgroundColor:[UIColor tt_redMoneyColor]];
    [fxline mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.bottom.width.equalTo(ckline);
        make.left.equalTo(_fxButton.mas_right).offset(1);
    }];
    
    _customerButton = [self createMessageButtonWithNomalImage:nil andTitle:@"消费用户0人" andTag:3332 andAction:@selector(clicksButton:)];
    [_middleView addSubview:_customerButton];
   ...
   //点击的时候把刷新清空
    ...
    switch (button.tag - 3330) {
        ...
        case 1: //分销
        {
            ...
            _string = @"1";
             [self anamationWithLeftx:SCREEN_WIDTH/3+10];
        }
            break;
        case 2:  //顾客
        {
            _customerButton.selected = YES;
            _ckButton.selected = NO;
            _fxButton.selected = NO;
            
            [_customerButton setUserInteractionEnabled:NO];
            [_ckButton setUserInteractionEnabled:YES];
            [_fxButton setUserInteractionEnabled:YES];
            _string = @"2";
            [self anamationWithLeftx:SCREEN_WIDTH*2/3+10];
