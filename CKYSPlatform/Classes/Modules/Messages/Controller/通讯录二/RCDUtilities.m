...
//        for (id user in userList) {
            ...
            if ([user isMemberOfClass:[PTContactorModel class]]) {
                PTContactorModel *userInfo = (PTContactorModel *)user;
                if (userInfo.remark.length > 0 && ![userInfo.remark isEqualToString:@""]) {
                    firstLetter = [self getFirstUpperLetter:userInfo.remark];
                } else {
                    if (IsNilOrNull(userInfo.name)) {
                        firstLetter = @"#";
                    }else{
