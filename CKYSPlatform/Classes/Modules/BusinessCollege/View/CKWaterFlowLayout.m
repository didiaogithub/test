...
- (void)prepareLayout {
    ...
    layoutHeader.frame = CGRectMake(0,0, SCREEN_WIDTH, AdaptedHeight(300));
...
//    layoutFooter.frame = CGRectMake(0,20, SCREEN_WIDTH, self.sectionInset.top);
...
- (UICollectionViewLayoutAttributes *)layoutAttributesForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.itemHeightBlock) itemHeight = self.itemHeightBlock(itemWidth, indexPath);
        ...
        if ([self.delegate respondsToSelector:@selector(waterFlowLayout:itemHeightForWidth:atIndexPath:)])
            itemHeight = [self.delegate waterFlowLayout:self itemHeightForWidth:itemWidth atIndexPath:indexPath];
    }
